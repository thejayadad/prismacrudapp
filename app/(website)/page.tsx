...
import { Student } from "@prisma/client"; // ✅ Import Prisma type

export default async function Home() {
  const students: Student[] = await prisma.student.findMany(); // ✅ Typed result
          ...
          <div className="modal" role="dialog">
            <div className="modal-box">
              ...
              <form className="py-2 flex flex-col gap-4" action={createStudent}>
                <input
                  placeholder="Add the Kid's Name"
                  id="name"
                  name="name"
                  type="text"
            ...
            {students.map((student: Student) => (
              <div className="flex justify-between items-center w-full p-2 bg-amber-300" key={student.id as string}>
                ...
                <div className="flex items-center space-x-2">
                  <div>
                    <label htmlFor={`edit_modal_${student.id}`} className="btn btn-sm">Edit</label>

                    {/* Edit Modal (with unique ID per student to avoid toggle conflicts) */}
                    <input type="checkbox" id={`edit_modal_${student.id}`} className="modal-toggle" />
                    <div className="modal" role="dialog">
                      <div className="modal-box">
                        <h3 className="text-lg font-bold">Update Student</h3>
                        <form className="flex flex-col gap-2" action={updateStudent}>
                          <input
                            id="id"
                            name="id"
                        ...
                        <div className="modal-action">
                          <label htmlFor={`edit_modal_${student.id}`} className="btn">Close!</label>
                        </div>
                      </div>
                    </div>
                  </div>
